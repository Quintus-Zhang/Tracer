...
.. math::
    ...
    V_t(COH_t) = Max_{C_t \ge 0} [U(C_t) + \beta p_tE_t(V_{t+1}(COH_{t+1}))],\ \ for\ t\lt T
...
:math:`COH_{t+1}` is given by the agent's cash-on-hand accumulation equation:

.. math::

    COH_{t+1} = (1+R)(COH_t-C_t) + Y_{t+1}
